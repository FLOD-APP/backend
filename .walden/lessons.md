...
### 2026-05-14T10:00:00Z | foodics-order-id-client-generated | design

- Trigger: Foodics API v5 requires a **client-generated UUIDv4** as the `id` field in `POST /orders`. Foodics does NOT generate order IDs.
- Lesson: Always generate the order ID on the FLOD side before calling `POST /orders`. This also serves as an idempotency key — if the request is retried, Foodics rejects duplicates.
- Guardrail: The order builder must set `id: crypto.randomUUID()` (or accept an externally-provided ID for testability). Never rely on Foodics to generate the ID.

### 2026-05-14T10:01:00Z | foodics-rate-limit-escalation | design

- Trigger: Foodics rate limiting has an escalation mechanism — 10+ consecutive 429s within 10 seconds triggers a 60-second IP block.
- Lesson: Exponential backoff with jitter is essential. Never retry immediately on 429. Always respect the `retry-after` header.
- Guardrail: Token bucket rate limiter must cap at 30 req/min. On 429, wait at least the `retry-after` duration before next request. Add jitter to prevent thundering herd on retry.

### 2026-05-14T10:02:00Z | foodics-payment-type-7-only | design

- Trigger: When creating orders via API, the `payments` array only accepts type 7 (3rd party) or type 8 (External) payment methods. Type 5 (House Account) is system-managed and cannot be used in API order creation.
- Lesson: FLOD must create a type 7 payment method ("FLOD Subscription") during onboarding and use its ID for all API-created orders. House Account is a POS-side concept, not an API payment type.
- Guardrail: Never use payment type 5 in `POST /orders`. Always use the configured type 7 `FOODICS_PAYMENT_METHOD_ID`.

---

## Process / Workflow

### 2026-05-14T10:03:00Z | frontend-vat-double-counting | execute
